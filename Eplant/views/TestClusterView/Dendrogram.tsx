...
// Convert TreeData obj to D3Node obj
...
const MetadataVisualizations = ({ 
  ...
  // Dynamically set bar widths based on metadata, but limit width to avoid full span bars
  ...
  return (
    <g transform={`translate(${x + LABEL_OFFSET + 100}, ${y - BAR_HEIGHT - BAR_SPACING})`}>
      ...
      <g transform={`translate(0, ${BAR_HEIGHT + BAR_SPACING})`}>
        {/* Grey background bar for the primary gene only */}
          <rect
            x={0}
            y={0}
            width={BAR_WIDTH} // Background bar for primary gene
            height={BAR_HEIGHT}
            fill="#e0e0e0"
            stroke="#999"
            strokeWidth={0.5}
          />
        {/* Foreground black bar with dynamic width */}
        <rect
          x={BAR_WIDTH / 2}
          y={0}
          width={(BAR_WIDTH / 2) * expressionLevel} // Limits the width based on expressionLevel value
          height={BAR_HEIGHT}
          fill="#000000"
        />
        {/* Center line for expression comparison */}
        <line
          x1={BAR_WIDTH / 2}
          y1={-1}
          x2={BAR_WIDTH / 2}
...
export const Dendrogram = () => {
  ...
  // Usememo for caching for efficient rendering
  ...
  // Generates the tree at a right-angle rather than curved lines
  ...
  // Draw nodes and draw SVG/node values
        ...
        {!node.children && (
          <>
            ...
            <MetadataVisualizations
              x={node.y + LABEL_OFFSET * 20} // Offset for alignment horizontally
              y={node.x - 10} // Offset for alignment vertically
