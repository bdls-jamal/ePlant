...
export const HeatMapViewObject = () => {
  ...
  const { data, isLoading } = useQuery<HeatMapViewerData>({
    ...
    queryFn: async () =>
      await HeatMapViewerLoader(geneticElement, setLoadAmount),
    ...
    queries: [
      {
        ...
        queryFn: async () => {
          console.log(
            `[HeatMap] 🔄 Fetching PLANT data for gene: ${geneticElement?.id}`
          )
          ...
          console.log(
            `[HeatMap] ✅ PLANT data fetched for gene: ${geneticElement?.id}`,
            result
          )
          return result
        },
      ...
      },
      {
        ...
        queryFn: async () => {
          console.log(
            `[HeatMap] 🔄 Fetching EXPERIMENT data for gene: ${geneticElement?.id}`
          )
          const result = await EFPViewerLoader(
            geneticElement,
            experimentEFPs,
          ...
          console.log(
            `[HeatMap] ✅ EXPERIMENT data fetched for gene: ${geneticElement?.id}`,
            result
          )
          return result
        },
      ...
      },
      {
        ...
        queryFn: async () => {
          console.log(
            `[HeatMap] 🔄 Fetching CELL data for gene: ${geneticElement?.id}`
          )
          const result = await cellEFPLoader(geneticElement, () => {})
          console.log(
            `[HeatMap] ✅ CELL data fetched for gene: ${geneticElement?.id}`,
            result
          )
          return result
        },
  ...
  useEffect(() => {
    ...
    groupInfo.groupPositions.forEach(({ group, position, width }) => {
      ...
      const hasData = loadedGenes.some(
        (geneData) => geneData.data[group].length > 0
      )
      if (hasData && width > 0) {
                ...
                d3.select('.heatmap-tooltip')
                  ...
                  .html(
                    `<strong>Gene:</strong> ${geneData.gene}<br/>
                                        ...
                                        <strong>Value:</strong> ${point.value.toFixed(
                                          2
                                        )}<br/>
  ...
  return {
    geneData: {
      ...
      data: {
         ...
         */
        plant:
          plant?.viewData?.flatMap((sample, i) =>
         ...
         */
        experiment:
          experiment?.viewData?.flatMap((sample, i) =>
            sample.groups.flatMap((g: EFPGroup) =>
              g.tissues.map((t: EFPTissue) => ({
                value: t.mean,
                sample: t.name,
